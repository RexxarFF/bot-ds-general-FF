...
class FunFernusRealtimeServer:
    ...
    async def _websocket(self, request: web.Request) -> web.StreamResponse:
        ...
        async def expire_socket() -> None:
            # Tickets are deliberately short lived. Closing the socket at expiry
            # forces the browser to obtain a fresh ticket through the authenticated
            # PHP session. This prevents a logged-out/stale browser tab from keeping
            # a privileged realtime channel indefinitely.
            delay = max(1.0, float(ticket.exp - time.time()))
            try:
                await asyncio.sleep(delay)
                if not ws.closed:
                    await ws.close(code=4001, message=b"Realtime ticket expired")
            except asyncio.CancelledError:
                raise
            except Exception:
                log.debug("Realtime expiry close failed: user_id=%s", ticket.user_id, exc_info=True)
    ...
    async def publish_notification(self, user_id: int, notification: dict[str, Any]) -> int:
        """Push a trusted notification generated by the embedded Discord bridge."""
        if not self.enabled or not self.started or user_id <= 0:
            return 0
