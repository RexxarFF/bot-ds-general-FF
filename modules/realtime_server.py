...
class FunFernusRealtimeServer:
    ...
    def __init__(self, config: RealtimeConfig | None = None) -> None:
        ...
        self._used_ticket_nonces: dict[str, int] = {}
    ...
    def _consume_ticket_nonce(self, ticket: Ticket) -> bool:
        """Reject replay of one short-lived WebSocket ticket.

        Every browser reconnect requests a fresh PHP ticket, so accepting one nonce
        only once prevents a copied URL from opening extra authenticated sockets.
        """
        now = int(time.time())
        if len(self._used_ticket_nonces) > 4096:
            self._used_ticket_nonces = {nonce: exp for nonce, exp in self._used_ticket_nonces.items() if exp >= now}
        if not ticket.nonce or ticket.nonce in self._used_ticket_nonces:
            return False
        self._used_ticket_nonces[ticket.nonce] = ticket.exp
        return True

    async def _expire_connection(self, connection: RealtimeConnection) -> None:
        delay = max(1.0, float(connection.ticket_exp - int(time.time()) + 2))
        try:
            await asyncio.sleep(delay)
            if not connection.ws.closed:
                await connection.ws.close(code=4001, message=b"Realtime ticket expired")
        except asyncio.CancelledError:
            raise
        except Exception:
            log.debug("Не удалось закрыть websocket после истечения ticket user_id=%s", connection.user_id, exc_info=True)
    ...
    async def _websocket(self, request: web.Request) -> web.StreamResponse:
        ...
        if ticket is None or not self._consume_ticket_nonce(ticket):
            raise web.HTTPUnauthorized(text="Invalid or already used realtime ticket")
        ...
        expiry_task = asyncio.create_task(self._expire_connection(connection), name=f"realtime-ticket-{ticket.user_id}")
    ...
    async def publish_messenger_event(self, recipients: Any, event: dict[str, Any]) -> int:
        """Publish one already-persisted messenger event from an internal worker."""
        if not self._started or not isinstance(event, dict):
            return 0
        packet = {"type": "messenger.event", "event": event, "server_time": int(time.time() * 1000)}
        return await self._fanout(recipients, packet)
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            self._used_ticket_nonces.clear()
